...
def normale_bidim(x, z, mu_x_mu_z_sig_x_sig_z_p): #Python3中使用x_y的形式代替(x，y)，使其类似于列表的形式，在调用的时候，使用x_y[index]的形式

    A = 1 / (2 * np.pi * mu_x_mu_z_sig_x_sig_z_p[2] * mu_x_mu_z_sig_x_sig_z_p[3] * np.sqrt((1 - ((mu_x_mu_z_sig_x_sig_z_p[4]) ** 2))))
    B = 1 / (2 * (1 - (mu_x_mu_z_sig_x_sig_z_p[4]**2)))
    C = ((x - mu_x_mu_z_sig_x_sig_z_p[0]) / mu_x_mu_z_sig_x_sig_z_p[2]) ** 2
    D = 2 * mu_x_mu_z_sig_x_sig_z_p[4] * ((x - mu_x_mu_z_sig_x_sig_z_p[0]) * (z - mu_x_mu_z_sig_x_sig_z_p[1]) / (mu_x_mu_z_sig_x_sig_z_p[2] * mu_x_mu_z_sig_x_sig_z_p[3]))
    E = ((z - mu_x_mu_z_sig_x_sig_z_p[1]) / mu_x_mu_z_sig_x_sig_z_p[3]) ** 2
